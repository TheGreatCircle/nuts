...
function Nuts(opts) {
  ...
  this.router.get("/update/:platform/:version/:arch", this.onUpdate)
  ...
  this.router.get("/update/:platform/:version/:arch/RELEASES", this.onUpdateWin)
  this.router.get(
...
const filterPlatforms = (version, arch) => {
  if (arch === "ia32") {
    return {
      ...version,
      platforms: version.platforms
        .filter(currentVersion => currentVersion.filename.includes(arch))
        .map(platform => {
          if (platform.filename === "RELEASES-ia32") {
            return {
              ...platform,
              type: "windows_32",
              filename: "RELEASES"
  ...
  return {
    ...version,
    platforms: version.platforms.filter(
      currentVersion => !currentVersion.filename.includes("ia32")
    )
  }
}
...
Nuts.prototype.onDownload = function(req, res, next) {
  ...
  if (!filename) {
    ...
    if (arch && arch !== "ia32")
      return next(
        new Error(
          "The given architecture is not valid, don't specify an architecture if it is different of ia32"
        )
      )
  } else {
    platform = null
  ...
  this.versions
    ...
    .then(function(version) {
      ...
      if (filename) {
        ...
        var filteredVersion =
          arch === "ia32"
            ? filterPlatforms(version, arch)
            : filterPlatforms(version, "")
        console.log(filteredVersion)
        asset = platforms.resolve(filteredVersion, platform, {
...
Nuts.prototype.onUpdate = function(req, res, next) {
  if (req.params.arch && req.params.arch === "RELEASES") {
    next()
    return
  }
  var that = this
  ...
  var arch = req.params.arch ? "&arch=" + req.params.arch : ""
  ...
  Q()
    ...
    .then(function(versions) {
      ...
      gitFilePath = req.params.arch ? gitFilePath + "../" : gitFilePath
      res.status(200).send({
        url: urljoin(
          fullUrl,
          gitFilePath,
  ...
  that
    ...
    .then(function(versions) {
      ...
      var asset = _.find(latest.platforms, {
        filename: arch === "ia32" ? "RELEASES-ia32" : "RELEASES"
      })
